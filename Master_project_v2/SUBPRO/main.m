...
switch cont
    case 1
        ...
        while k < 5
            ...
            [psi0_mat, psi_end, mean] = ...
                getDistributionAndMean...
        ...
        % Plotting
            ...
            % 1/2 tasks
            hfig = plotMean(mean, tau, fontProps, colorMatrix);
            hLegend = legend(legendArray);
            set(hLegend, fontProps, 'Location', 'NorthEastOutside');
            % Save mean plot?
%             savePlot = lower(input('Save mean plot ("y" or "n"): ', 's'));
...
%             end %if
            ...
            % 2/2 tasks
            hFig = ...
                plotFinalDistribution(psi_struct, fontProps, colorMatrix, 0);
            legendArray{end+1} = sprintf('Equilibrated distributions');
            hLegend = legend(legendArray);
            set(hLegend, fontProps, 'Location', 'NorthEastOutside');
            % Save final distros?
%             savePlot = lower(input('Save final distros plot ("y" or "n"): ', 's'));
%             if strcmp(savePlot, 'y')
%                 plot_name_handle = input('Enter filename: ', 's');
%                 saveas(hFig, plot_name_handle, 'png')
%             end %if
...
%         end %if
        ...
        hFig = ...
                plotFinalDistribution(psi_struct, fontProps, colorMatrix, 1);
        legendArray{end+1} = sprintf('Initial distribution');
        hLegend = legend(legendArray);
        set(hLegend, fontProps, 'Location', 'NorthEastOutside');
        % Save final distribution plot?
%         savePlot = lower(input('Save final distributions plot ("y" or "n"): ', 's'));
%         if strcmp(savePlot, 'y')
%             plot_name_handle = input('Enter filename: ', 's');
%             saveas(hFig, plot_name_handle, 'png')
%         end %if
