...
kc2   = 0;
ratio = 0;
beta  = [kb1, kb2, ratio, kc2];
lb    = [1e-8, 1e-5, 0.1,   1e-2];
ub    = [1e0,  1,    100,   1e4];
...
%    'FunctionTolerance', 1e-12);
objfunHandle = @(beta) objectiveFun(beta, r);
[beta,fval,exitflag,output] = ...
    particleswarm(objfunHandle, length(beta), lb, ub, options);
...
now = strcat('Results/Experimental/crudeB/', now{1}, '-', now{2});
%save(now)
