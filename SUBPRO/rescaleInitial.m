...
function [ fn, fv, r, t ] = rescaleInitial( f0, phi, flag )
...
t      = split(A.textdata(3:end, 3));
t      = split(t(:, 2), ':');
hours  = str2double(t(:, 1));
mins   = str2double(t(:, 2));
hours  = hours - hours(1);
mins   = mins - mins(1);
t      = hours * 3600 + mins * 60;
for i  = 1:length(t)-1
    if t(i+1)  == t(i)
        t(i+1) = t(i+1) + 25; %add 25 seconds to make them unique
    end %if
end %for
